...
export async function GET(req: NextRequest) {
  try {
    ...
    const validatedQuery = getCollaborationQuerySchema.parse(query);

    const result = await CollaborationService.getAll(validatedQuery);

    return NextResponse.json(
      { message: "Collaborations retrieved successfully", ...result },
      { status: 200 }
    );
  } catch (error: any) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { message: "Collaboration created successfully", data: newCollaboration },
      { status: 201 }
    );
  } catch (error: any) {
